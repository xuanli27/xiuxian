      ...
      {activeTab === 'HALL' ? (
        ...
        <div className="animate-in fade-in slide-in-from-right-4 duration-300">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {SHOP_ITEMS.map((item) => (
              <Card key={item.id} className="flex flex-col justify-between hover:border-secondary-500/50 transition-colors">
                <div className="flex gap-4">
                  <div className="text-4xl bg-surface-900 p-3 rounded-xl h-fit">{item.icon}</div>
                  <div>
                    <h3 className="font-bold text-lg text-content-100">{item.name}</h3>
                    <p className="text-sm text-content-400 mt-1">{item.description}</p>
                    <div className="mt-2 flex items-center gap-2 text-xs text-secondary-400 bg-secondary-500/10 w-fit px-2 py-1 rounded">
                ...
                <div className="mt-4 pt-4 border-t border-border-base flex justify-between items-center">
                  ...
                  <Button
                    size="sm"
                    variant="secondary"
                    disabled={player.contribution < item.price || purchase.isPending}
