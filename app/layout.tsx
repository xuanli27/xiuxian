...
import { Home, ListTodo, Backpack, Crown, Armchair, User, Moon, Sun, Cpu, ScrollText } from 'lucide-react';
import { Modal } from '../components/ui';
import clsx from 'clsx';

export default function RootLayout({ children }: { children: React.ReactNode }) {
  const { view, player, offlineReport, setTheme, setView, clearOfflineReport } = useGameStore();
  
  // Apply Theme & Title
  useEffect(() => {
    document.documentElement.setAttribute('data-theme', player.theme);
    document.title = `摸鱼修仙录 | Rank: ${player.rank}`;
  }, [player.theme, player.rank]);
  ...
  return (
    <div className="min-h-screen bg-surface-950 text-content-100 font-serif selection:bg-primary-500/30 pb-24 transition-colors duration-700 relative overflow-hidden">
      ...
      {/* Top Navbar */}
      {showNav && (
        <nav className="fixed top-0 w-full z-40 px-4 py-3 flex justify-between items-center animate-in fade-in slide-in-from-top-4 duration-500">
      ...
      {showNav && (
        <div className="fixed bottom-6 left-4 right-4 z-40 flex justify-center animate-in fade-in slide-in-from-bottom-8 duration-700">
